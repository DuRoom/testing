...
namespace DuRoom\Testing\integration\Extension;
...
class ExtensionManagerIncludeCurrent extends ExtensionManager
{
    ...
    public function getExtensions()
    {
        ...
        if (Arr::get($package, 'type') === 'duroom-extension') {
            ...
            $this->extensions = $extensions->sortBy(function ($extension, $name) {
                return $extension->composerJsonAttribute('extra.duroom-extension.title');
            });
        }
     ...
     * This is resolved dynamically because DuRoom's filesystem configuration
     * might not be booted yet when the ExtensionManager singleton initializes.
     */
    protected function getAssetsFilesystem(): Cloud
    {
        return new FilesystemAdapter(new FlysystemFilesystem(new Local($this->paths->public . '/assets'), ['url' => resolve('duroom.config')->url() . '/assets']));
    }
}
